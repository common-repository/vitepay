=== VITEPAY ===
Contributors: logineo
Donate link: N/A
Tags: vitepay, mali, bamako, orange, money, cfa, xof, bceao
Requires at least: 4.3.1
Tested up to: 5.4.2
Stable tag: 5.4.2
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Le tout premier service de paiement sur internet du Mali.

== Description ==

La société Logineo “keep IT simple” en collaboration avec Orange Money vous présente Vitepay,

le tout premier service de paiement sur internet du Mali.

Vitepay, passant par le réseau Orange Money pour effectuer les transactions, donne la possibilité à vos clients de payer directement sur votre site web leurs achats de manière simple, rapide et sécurisée. Que ce soit pour la vente de produits ou de services, vos clients pourront desormais vous regler sans se deplacer.

The company Logineo "keep IT simple" in collaboration with Orange Money presents Vitepay,

The first online payment service of Mali.

Vitepay , through the Orange Money Network to complete transactions , provides an opportunity for your customers to pay directly on your website in a simple , fast and secure way. Whether selling products or services, your customers will henceforth settle from anywhere.

== Installation ==

1. Upload des fichiers dans le repertoire `/wp-content/plugins/`
1. Activer le plugin dans le menu « plugins » de wordpress
1. Se connecter sur store.vitepay.com et récupérer les accès a l’api
4. se connecter sur l’interfaces d’administration de woocommerce et entrer les configurations dans les champs requis

pour plus d’information www.vitepay.com/devellopers

== Frequently Asked Questions ==

= Est il possible de payer avec autres choses que Orange money sur la plateforme? =

- Nous nous sommes concentrés sur Orange Money pour le Mali, bientôt d’autres moyens de paiement suivront.

An answer to that question.

== Screenshots ==

1. Exemple d’interface d’accès au plugin a travers l’interface woo commerce. (/assets/screenshot-1.png)
2. Exemple d’interface de configuration du plugin. (/assets/screenshot-2.png)
3. Exemple de pastille de paiement. (/assets/screenshot-3.png)
4. Exemple d’interface de paiement Vitepay

== Changelog ==

= 1.4 =
* Correction de bug pour plus de stabilité

= 1.2 =
* Correction de bug pour plus de stabilité

= 1.1 =
* Correction de bug pour plus de stabilité
* Ajout de la verification de la devise du panier
* Amelioration de l’UI
* Ajout du système de logs

= 1.0 =
* Ajout du Hash pour plus de sécurité
* changement mineure et corrections de bugs

= 0.5 =
* Version Initiale Beta 1

== Upgrade Notice ==

= 1.1 =
Correction de bug pour plus de stabilité

= 1.0 =
Les mises à jour doivent absolument être automatique pour éviter les différences de version avec la plateforme Vitepay et pour sécurisé vos transactions.

= 0.5 =
Les mises à jour doivent absolument être automatique pour éviter les différences de version avec la plateforme Vitepay et pour sécurisé vos transactions.

== Arbitrary section ==

Merci de faire confiance au réseau Vitepay partenaires de Orange money sur le paiement en ligne.

== A brief Markdown Example ==

Ordered list:

1. Active le paiement en ligne à travers le réseau Orange money


Unordered list:

* Facile a intégrer
* sécurisé
* Fiable

Here's a link to [Logineo](http://logineo.com/ "Your favorite software Maker") and one to [Markdown's Syntax Documentation][markdown syntax].
Titles are optional, naturally.

[markdown syntax]: http://vitepay.com
            "Markdown is what the parser uses to process much of the readme file"

Markdown uses email style notation for blockquotes and I've been told:
> Asterisks for *emphasis*. Double it up  for **strong**.

`<?php code(); // goes in backticks ?>`

